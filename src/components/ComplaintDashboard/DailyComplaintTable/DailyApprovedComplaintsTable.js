...
const DailyApprovedComplaintsTable = ({
  handleOpenModal,
  openRemarksModal,
  calculateAgingDays,
  getStatusClass,
  ...
  // Filters (approvedDate locked to today)
  ...
  // Dropdown filter values (from all complaints)
  ...
  const fetchComplaints = async () => {
    ...
    const params = {
      ...
      status: "Approved",
      page: 0,
      size: 2000 // Large enough for all for today
    };
  ...
  // Flattened and filtered for "open and approved same day"
  function complaintMatchesSameDay(complaint) {
    const approvedDate = complaint.approvedDate ? complaint.approvedDate.slice(0, 10) : "";
  ...
  // Table action handlers, Excel export, clear filters, etc
  const handleExportExcel = async () => {
    ...
    await generateExcelReport(
      exportList,
      "Approved (Same Day Open+Approved)",
      API_BASE_URL,
      uniqueCities,
      "standard"
    );
  ...
  const shouldHighlightRow = (complaint) => {
    ...
    const diffDays = Math.ceil(
      (new Date() - new Date(complaint.approvedDate)) / (1000 * 60 * 60 * 24)
    );
    return diffDays > 3;
  };
  ...
  return (
    <div>
      {/* Filters bar, approvedDate locked to today */}
      <ComplaintFilters
        filters={filters}
        onFiltersChange={(f) => setFilters({ ...f, approvedDate: effectiveDate })}
        ...
        dateLocked={true}
        approvedDateLocked={true}
        approvedDateLabel="(Same-day Open & Approved Only)"
      />
          ...
          <table className="complaint-table">
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan="21" style={{ textAlign: "center" }}>
                    No daily approved complaints to display
                  </td>
                </tr>
              )}
            </tbody>
...
export default DailyApprovedComplaintsTable;
