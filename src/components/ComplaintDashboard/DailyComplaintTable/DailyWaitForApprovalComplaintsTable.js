...
const DailyWaitForApprovalComplaintsTable = ({
  handleOpenModal,
  openRemarksModal,
  calculateAgingDays,
  getStatusClass,
  ...
  // Filters (quotationDate locked to today)
  ...
  // Fetch grouped complaints from backend
  const fetchComplaints = async () => {
    ...
    const params = {
      ...
      status: "Wait For Approval",
      page: 0,
      size: 2000 // Large enough to fetch all today's
    };
  ...
  // Only same-day opened & quotation date
  function complaintMatchesSameDay(complaint) {
    const quotationDate = complaint.quotationDate ? complaint.quotationDate.slice(0, 10) : "";
    const openDate = complaint.date ? complaint.date.slice(0, 10) : "";
    return (
      complaint.complaintStatus === "Wait For Approval" &&
      quotationDate === effectiveDate &&
  ...
  // Highlight row if quotationDate > 3 days old
  ...
  const handleExportExcel = async () => {
    ...
    await generateExcelReport(
      exportList,
      "Wait For Approval (Same Day Open+WaitForApproval)",
      API_BASE_URL,
      uniqueCities,
      "standard"
    );
  ...
  return (
    <div>
      <ComplaintFilters
        filters={filters}
        onFiltersChange={(f) => setFilters({ ...f, quotationDate: effectiveDate })}
        ...
        approvedDateLabel="(Same-day Open & Wait For Approval Only)"
      />
          ...
          <table className="complaint-table">
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan="21" style={{ textAlign: "center" }}>
                    No daily Wait For Approval complaints to display
                  </td>
                </tr>
              )}
            </tbody>
...
export default DailyWaitForApprovalComplaintsTable;
